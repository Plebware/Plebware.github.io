...
* **`grep`, `cut`, `tr`, `xargs`**: Used to parse and sanitize settings from `state.conf`.
...
| **Specialized & Flatpaks** | `info.bibletime.BibleTime` (via `flatpak`), `evince` |
